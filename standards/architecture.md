...
├── seed.sql                  # D1 seed data (npm run seed:local)
...
one of them `migrate:remote` costs all three a lookup and gets skipped in CI.
...
**A script that could sensibly exist for more than one environment names the
one it targets — always, including the local one. A script with only one
possible target carries no suffix.** So `dev`, `build`, `check`, `typecheck`,
`test` and `verify:serving` stay bare, and every deploy, migrate and seed is
suffixed: there is no bare `deploy`, no bare `db:migrate`, no bare `seed`. A
product that grows a staging environment adds `deploy:staging` and
`db:migrate:staging` beside the production ones ([ops §9](ops.md)); nothing
already written changes meaning. — *Why:* across a portfolio the same bare name
drifts to opposite meanings — `db:migrate` applying to a laptop in one repo and
